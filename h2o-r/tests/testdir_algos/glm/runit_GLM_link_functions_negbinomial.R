...
#	for (thetaO in thetaSet) {
	...
	rglm <- glm(formula=R.formula, data=R.data[,2:9], family=negative.binomial(link=identity, theta=thetaO), na.action=na.omit)
	browser()
	h2oglm <- h2o.glm(x=myX, y=myY, training_frame=h2o.data, family="negbinomial", link="identity",alpha=0.5, lambda=0, nfolds=0, optimize_theta=FALSE, theta=thetaO)
	compareModels(h2oglm, rglm)
	
	print("Create models with canonical link: LOG")
	h2oglm <- h2o.glm(x=myX, y=myY, training_frame=h2o.data, family="negbinomial", link="log",alpha=0.5, lambda=0, nfolds=0, optimize_theta=FALSE, theta=thetaO)
	rglm <- glm(formula=R.formula, data=R.data[,2:9], family=negative.binomial(link=log, theta=thetaO), na.action=na.omit)
  compareModels(h2oglm, rglm)
#}
}
